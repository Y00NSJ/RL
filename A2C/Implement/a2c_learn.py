...
class A2Cagent(object):
    ...
    def load_weights(self, path):
        self.actor.load_weights(path + 'pendulum_actor.weights.h5')
        self.critic.load_weights(path + 'pendulum_critic.weights.h5')
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            while not done:
                ...
                # 배치에 저장
                batch_state.append(state_batch)
                batch_action.append(action_batch)
                batch_reward.append(train_reward)
                batch_next_state.append(next_state_batch)
                batch_done.append(done_batch)
    ...
    def plot_result(self):
        rewards = np.array(self.save_epi_reward).flatten()
        plt.plot(rewards)
